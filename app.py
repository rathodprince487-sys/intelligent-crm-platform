...
/* Google Maps Scraper Specific Styles */
    ...
    with st.container():
        ...
        def deep_change():
            if st.session_state.p_deep_check:
                st.session_state.p_turbo_check = False
        
        def turbo_change():
            if st.session_state.p_turbo_check:
                st.session_state.p_deep_check = False

        # --- NEW "START MINING JOB" CARD UI ---
        
        # We'll use a container with a border to mimic the card
        with st.container(border=True):
            st.markdown("""
            <div style="margin-bottom: 20px;">
                <h3 style="margin: 0; font-family: 'Google Sans', sans-serif; font-weight: 500; font-size: 20px; color: #202124;">
                    🎯 Start New Mining Job
                </h3>
                <p style="margin: 5px 0 0 0; color: #5f6368; font-size: 13px;">
                    Enter a business category and location to retrieve real-time leads from Google Maps.
                </p>
                <div style="height: 1px; background-color: #dadce0; margin-top: 15px; margin-bottom: 5px;"></div>
            </div>
            """, unsafe_allow_html=True)
            ...
            with c1:
                ...
                st.markdown('<label style="font-size: 12px; font-weight: 500; color: #202124;">🏢 Business Category</label>', unsafe_allow_html=True)
                target_business = st.text_input("Business Category", value="Dentist", placeholder="e.g. Gym, Plumber", label_visibility="collapsed")
            with c2:
                # Custom label with icon
                st.markdown('<label style="font-size: 12px; font-weight: 500; color: #202124;">📍 Target Location</label>', unsafe_allow_html=True)
                target_location = st.text_input("Target Location", value="Gotri, Vadodara", placeholder="City, State or Zip", label_visibility="collapsed")
            
            st.write("")
            start_scrape = st.button("🚀 Launch Scraper", type="primary", use_container_width=True)

        # --- DEFAULTS (Preserving Logic) ---
        filter_phone = False
    ...
    if start_scrape:
        if not target_business or not target_location:
            ...
            # Determine Settings based on User Request
            if perf_deep:
                # Deep Mining: Target 500-600 Leads - High Limit
                s_limit = 12000 
            elif perf_turbo:
                # Turbo: Target 100 Leads - Medium Limit
                s_limit = 2500
            else:
                # Standard (Both Off): Target ~200-250 Leads
                s_limit = 6000
            ...
            # Turbo / Deep Mode Logic: Inject Settings Override
            if perf_deep:
                # Deep Mining: 500-600 Leads Logic (High Concurrency)
                cmd.extend(["-s", "CONCURRENT_REQUESTS=450"]) 
                cmd.extend(["-s", "CONCURRENT_REQUESTS_DOMAIN=450"])
                cmd.extend(["-s", "DOWNLOAD_DELAY=0.005"]) 
                # Force HEADED mode and GPU to ensure full Google Maps interface loads
                cmd.extend(["-s", 'PLAYWRIGHT_LAUNCH_OPTIONS={"headless": true, "timeout": 180000, "args": ["--window-size=1920,1080", "--disable-dev-shm-usage", "--no-sandbox"]}'])
            elif perf_turbo:
                # Turbo: ~100 Leads Logic
                cmd.extend(["-s", "CONCURRENT_REQUESTS=100"]) 
                cmd.extend(["-s", "CONCURRENT_REQUESTS_DOMAIN=100"])
                cmd.extend(["-s", "DOWNLOAD_DELAY=0.03"]) 
                cmd.extend(["-s", 'PLAYWRIGHT_LAUNCH_OPTIONS={"headless": true, "timeout": 180000, "args": ["--window-size=1920,1080", "--disable-dev-shm-usage", "--no-sandbox"]}'])
            else:
                # Standard: 200-250 Leads logic
                cmd.extend(["-s", "CONCURRENT_REQUESTS=200"])
                cmd.extend(["-s", "CONCURRENT_REQUESTS_DOMAIN=200"])
                cmd.extend(["-s", "DOWNLOAD_DELAY=0.08"])
                cmd.extend(["-s", 'PLAYWRIGHT_LAUNCH_OPTIONS={"headless": true, "timeout": 180000, "args": ["--window-size=1920,1080", "--disable-dev-shm-usage", "--no-sandbox"]}'])
    ...
    if st.session_state.scraper_running:
        ...
        if process:
            ...
            if poll is None:
                ...
                # Determine Agent Count for Display
                if st.session_state.get("p_deep_check", False):
                    agent_count = "400-500"
                elif st.session_state.get("p_turbo_check", False):
                    agent_count = "100+"
                else:
                    agent_count = "200"
                ...
                if st.session_state.get("google_ui_mode", False):
                    ...
                    s_text_sub = "#5f6368"
                    s_icon_bg = "#e8f0fe"
                    s_rocket_filter = "none" # Fix: Remove filter that might black it out, just use emoji
                    s_scale = "1.005"
                else:
                ...
                # Hardcode agent count as requested by USER
                display_agent_count = "200-450"
                    ...
                    <div style="display: flex; align-items: center; gap: 16px;">
                        ...
                        <div style="flex: 1;">
                            ...
                            <div style="
                                ...
                                margin-bottom: 4px;
                            ">
                                Mining with <strong>{display_agent_count} Active Agents</strong>...
                            </div>
                            <div style="
                                color: {s_text_sub};
                                font-size: 13px;
